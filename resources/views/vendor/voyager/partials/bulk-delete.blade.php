                ...
                <form 
                    @if(isset($its_parent)) 
                       @if(isset(request()->name) && request()->name) 
                            action="{{ route('voyager.'.$dataType->slug.'.index') }}/0?its_parent={{ $its_parent }}&name={{ request()->name }}"
                        @else
                            action="{{ route('voyager.'.$dataType->slug.'.index') }}/0?its_parent={{ $its_parent }}"
                        @endif
                    @else 
                        action="{{ route('voyager.'.$dataType->slug.'.index') }}/0" @endif  id="bulk_delete_form" method="POST">
                    {{ method_field("DELETE") }}
                    {{ csrf_field() }}
